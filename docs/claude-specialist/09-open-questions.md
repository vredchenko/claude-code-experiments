...
This document catalogs unresolved design decisions that need answers before or
...
- **D. Local by default, export on demand**: Gitignored, but can generate
  reports
...
**Current thinking**: Start with A (Fully gitignored), add C (Opt-in) later if
needed.
...
**Current thinking**: Start with B (Categorized sections), enforce structure.
Consider D (Tiered) if it grows too large.
...
**Current thinking**: Start conservative (higher thresholds), tune down based on
acceptance rates.
...
- **C. Git history analysis**: Specialist Agent analyzes git history for
  patterns
...
**Current thinking**: Start with A+C (Session-local + Specialist Agent
analysis), add B (Persistent state) in Phase 3.
...
**Current thinking**: Use proposed strategy, adjust based on participation
rates.
...
**Current thinking**: Start with D (Periodic review), consider B (Inactivity)
...
**Current thinking**: A (Always ask) + quality control (contradiction detection
flags for review).
...
- **C. Shared core + personal additions**: CLAUDE.md shared, commands/agents
  personal
...
**Current thinking**: Start with C (Shared core + personal additions). CLAUDE.md
version controlled, usage-stats.json gitignored.
...
**Current thinking**: B (Warn but continue) for most hooks, A (Block) only if
...
**Current thinking**: User-initiated primarily, optional weekly/monthly
automated summaries.
...
**Concept**: After 1-2 occurrences, predict if pattern will continue and suggest
tool creation proactively.
...
**Concept**: General patterns (testing strategies, common commands) could
...
**Question**: Can we automatically test if new tools/patterns are actually
beneficial?

**Concept**: After creating a tool, measure if it's actually used, saves time,
reduces errors.
...
**This document is living**: Questions will be added, answered, and new ones
will emerge.
