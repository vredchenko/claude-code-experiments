...
async function main() {
  ...
  const services: ServiceConfig[] = [
      ...
      healthCheck: async () => {
        try {
          const response = await fetch(
            `http://localhost:${env.MINIO_PORT || "9000"}/minio/health/live`
          );
          return response.ok;
        } catch {
          return false;
    ...
    {
      name: "SurrealDB",
      port: parseInt(env.SURREALDB_PORT || "8000", 10),
    ...
    {
      name: "SurrealMCP",
      port: parseInt(env.SURREALDB_MCP_PORT || "3004", 10),
  ...
  console.log(
    "   3. Use MinIO, Redis, and SurrealDB through Claude Code (with official SurrealMCP)"
  );
  console.log("");
  ...
  console.log("   bun run start                # Run this setup script");
}
